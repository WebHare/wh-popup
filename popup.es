var moo = require('@webhare-system/frameworks/mootools/core')
var $ = moo.$;
var Element = moo.Element;
var Class = moo.Class;
var CSSTransitionManager = require('./csstransitionmanager.es');
var legacybase = require('@webhare-system/internal/legacybase')
...
How to use:
  ...
  Opening a dialog:
    ...
    var popup = PopupManager.createFromElement("elementid", options);
    var popup = PopupManager.createFromURL("url", options);
  ...
  Templating & Styling
    ...
    PopupManager.setDefaultPopupOptions({ theme: "mytheme", explicitclose: true, closebutton: false });
...
let PopupManagerClass = new Class(
...
, createFromElement: function(element, settings)
  {
    ...
    if (!popup)
    {
      ...
      popup = new BasicPopup(element, combinedsettings);
...
, createFromURL: function (url, settings)
  {
    ...
    if (!popup)
    {
      ...
      popup = new WebviewerPopup(settings.id ? settings.id : null, combinedsettings);
    }
...
, getPopupContainingNode: function(node)
  ...
  ///////////////////////////////////////////////////////////////
  //
  //  Semi-Private (for use in classes based on BasicPopup)
  //
...
, removeFromStack: function(instance)
  {
    ...
    if (modalidx > -1)
    {
      ...
      else
      {
        ...
        PopupManager.fireEvent("activate", { popup: this });
...
, getPopupIndex: function(popup_id_or_ref)
  {
    ...
    if (typeof(popup_id_or_ref) == "string")
      key = "id";
    else if (popup_id_or_ref instanceof BasicPopup)
      key = "instance"
...
, __computePopupOptions: function(node, options, options_attribute)
  {
    ...
    var options_from_attribute = this.parsePopupOptionsString(options_str, BasicPopup.prototype.options);
...
, parsePopupOptionsString: function(optionsstring, options_prototype)
  {
    ...
    for (var tel=0; tel<options_arr.length; tel++)
    {
      ...
      {
        var optiontype = typeof BasicPopup.prototype.options[keyval[0]];
...
, onAnchorClick: function(evt)
  {
    ...
    var settings_attribute = this.parsePopupOptionsString(settings_str, WebviewerPopup.prototype.options);
...
let PopupManager = new PopupManagerClass();

$(window).addEvent('domready', () => PopupManager.initDOM() );
...
, initialize: function(popup_id_or_ref, options)
  {
    ...
    PopupManager.register(this, popup_id_or_ref);
    this.buildDOM();
...
, buildDOM: function()
  {
    ...
    */
    ...
    this.transitionmanager = new CSSTransitionManager(chrome, { onended: this.onTransitionsEnded.bind(this) });

    var btn_close = null;
    if (this.options.closebutton)
    {
      btn_close = new Element("div", { "class": "wh-popup-closebutton wh-popup-action-close" } );
    }

    var body_container = new Element("div", { "class": "wh-popup-body-container"});

    var body = new Element("div", { "class": "wh-popup-body" });
...
, show: function()
  {
    ...
    PopupManager.addToStack(this, this.options.modal);
...
*/
    ...
    PopupManager.fireEvent("beforeshow", { popup: this });
    ...
    this.transitionmanager.triggerTransition(function()
    {
      ...
      this.ensureFocus();
      legacybase.fireLayoutChangeEvent(this.nodes.container);
    }.bind(this));
  }
...
, hide: function()
  {
    ...
    PopupManager.fireEvent("beforehide", { popup: this });
    ...
    PopupManager.removeFromStack(this);
    var container = this.nodes.container;
    this.transitionmanager.triggerTransition(function()
    {
...
, destroy: function()
  ...
  /** @short to check whether a node is part of the popup.
             This can be used together with PopupManager's events,
...
, ensureFocus:function()
  {
    if(!this.nodes.container.contains(legacybase.getActiveElement(document)))
      this.focus();
  }
...
, onHideCompleted: function()
  {
    ...
    PopupManager.fireEvent("afterhide", { popup: this });
...
, onShowCompleted: function()
  {
    ...
    PopupManager.fireEvent("aftershow", { popup: this });
...
, initialize: function(popupid, options)
  {
    ...
    this.__setOptions(options);
    PopupManager.register(this, popupid);
    this.buildDOM();
...
*/
//PopupManager.modalitylayer.options.container =

let ModalityLayer = new Class(
{ Implements: [ Options, Events ]
, options:
...
, initialize: function(options)
  {
    ...
    if (this.options.overlaynode)
    {
      ...
      this.transitionmanager = new CSSTransitionManager(this.overlaynode, { onended: this.onTransitionsEnded.bind(this) });
    }
  }
...
, onModalityLayerClick: function()
  {
    ...
    var lastmodalpopup = PopupManager.modalstack.getLast();
...
, show: function(beforenode)
  {
    ...
    if(!this.overlaynode)
    {
      this.overlaynode = new Element("div", { "class": "wh-modalitylayer"
...
$(window).addEvent("domready", function()
  {
    //PopupManager.modalitylayer = new $wh.ModalityLayer({ background_blur_radius: 1, effectnode: document.body });
    PopupManager.modalitylayer = new $wh.ModalityLayer({ background_blur_radius: 4, effectnode: $("modality_wrapper") });
  });
*/
// Initialize the modalitylayer _after_ the TransitionManager class is defined
PopupManager.modalitylayer = new ModalityLayer();
//Popup.createFromElement = PopupManager.createFromElement.bind(PopupManager);
//Popup.createFromURL =  PopupManager.createFromURL.bind(PopupManager);

//Close topmost dialog
function closeTop()
{
  if(!PopupManager.popupstack.length)
    throw "No popups currently open";

  if(PopupManager.popupstack.length)
    PopupManager.popupstack.getLast().hide();
}
//Close all dialogs
function closeAll()
{
  while(PopupManager.popupstack.getLast())
    closeTop();
}
